...
const signAccessToken = (user) =>
  jwt.sign(
    { userID: user.UserID, fullName: user.FullName, roles: user.Roles, avatar: user.Avatar, facultyID: user.FacultyID },
...
const login = async (req, res) => {
  try {
    ...
    if (!username || !password) return badRequest(res, 'Vui lòng nhập tên đăng nhập và mật khẩu');
    ...
    if (needsUpgrade) {
      ...
      await execute(`UPDATE "User" SET "Password" = @newHash WHERE "UserID" = @userID`, { newHash, userID: user.UserID });
    }
    ...
    return success(res, {
      ...
      user: {
        ...
        Roles: user.Roles, FacultyID: user.FacultyID,
        FacultyName: user.FacultyName, Email: user.Email, Mobi: user.Mobi,
      },
...
const getMe = async (req, res) => {
  try {
    const users = await query(
      `SELECT u."UserID", u."FullName", u."Avatar", u."Roles", u."FacultyID",
              u."Email", u."Mobi", u."Visible", f."FacultyName"
       FROM "User" u
       LEFT JOIN Faculty f ON u."FacultyID" = f."FacultyID"
...
const refreshToken = async (req, res) => {
  try {
    ...
    const user = await queryOne(
      `SELECT "UserID","FullName","Avatar","Roles","FacultyID"
       FROM "User" WHERE "UserID" = @userID AND "Visible" = 1`,
    ...
    return success(res, { accessToken: signAccessToken(user) }, 'Làm mới token thành công');
  } catch {
    res.clearCookie('refreshToken');
...
const register = async (req, res) => {
  try {
    ...
    const existing = await queryOne(`SELECT "UserID" FROM "User" WHERE "UserID" = @username`, { username: username.trim() });
    ...
    await execute(
      ...
      {
        username: username.trim(), fullName: fullName.trim(), hashed,
        email: email?.trim() || '', mobi: mobi?.trim() || '',
