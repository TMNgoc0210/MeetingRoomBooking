...
const searchRoom = async (req, res) => {
  try {
    ...
    `;
    const params = {};
    if (roomName) { sql += ` AND r."RoomName" LIKE '%' || @roomName || '%'`; params.roomName = roomName; }
    if (areaID && parseInt(areaID) > 0) { sql += ` AND r."AreaID" = @areaID`; params.areaID = parseInt(areaID); }
  ...
  try {
    ...
    const result = await execute(
      `INSERT INTO Room ("RoomName","AreaID","Seat","PhoneCall","VideoCall","IsVIP","VIPCondition","VIPMinutes","Visible","Desc","Avatar","CreateBy","CreateDate")
       VALUES (@roomName, @areaID, @seat, @phoneCall, @videoCall, @isVIP, @vipCondition, @vipMinutes, @visible, @desc, @avatar, @createBy, TO_CHAR(NOW(), 'YYYY-MM-DD HH24:MI:SS'))`,
      {
        roomName, areaID: parseInt(areaID), seat: parseInt(seat) || 0,
        phoneCall: phoneCall === true || phoneCall === 'true' ? 1 : 0,
        videoCall: videoCall === true || videoCall === 'true' ? 1 : 0,
        isVIP: isVIP === true || isVIP === 'true' ? 1 : 0,
        vipCondition: parseInt(vipCondition) || 0, vipMinutes: parseInt(vipMinutes) || 60,
        visible: visible !== false ? 1 : 0, desc: desc || '',
  ...
  try {
    ...
    await execute(
       ...
       WHERE "RoomID" = @roomID`,
      {
        ...
        visible: visible !== false ? 1 : 0, desc: desc || '',
        avatar: avatar || '/uploads/images/nopic.png',
        createBy: createBy || req.user?.userID || 'admin', roomID,
      }
    );
...
const deleteRoom = async (req, res) => {
  try {
    await execute(`UPDATE Room SET "Visible" = 0 WHERE "RoomID" = @roomID`, { roomID: parseInt(req.params.id) });
...
module.exports = { getListRoom, getAllRoom, getDetailRoom, getRoomsByArea, searchRoom, searchAllRoom, addRoom, updateRoom, deleteRoom };
